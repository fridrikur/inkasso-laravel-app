<div class="space-y-6 relative min-h-[60vh]">
    ...
    {{-- INITIAL FULDSKÆRMS LOADER CENTRERET PÅ SKÆRMEN --}}
    @if (! $readyToLoad)
        <div wire:init="loadKreditorer" class="fixed inset-0 z-50 flex flex-col items-center justify-center bg-white/40 dark:bg-slate-900/40 backdrop-blur-md">
            <div class="bg-white/90 dark:bg-slate-900/90 rounded-3xl p-6 shadow-2xl text-center max-w-sm w-full mx-4 backdrop-blur-xl">
                <div class="mx-auto mb-4 h-12 w-12 animate-spin rounded-full border-4 border-indigo-500/20 border-t-indigo-500"></div>
                <h2 class="text-base font-bold text-slate-900 dark:text-white tracking-tight">Henter kreditoroversigt...</h2>
                <p class="mt-1 text-xs text-slate-500">Indlæser kreditorer, porteføljer og tilknyttede brugere.</p>
            </div>
        </div>
    @endif

    {{-- SELVE INDHOLDET (BLIVER SLØRET / DEAKTIVERET INDTIL $readyToLoad ER SAND) --}}
    <div class="space-y-6 transition duration-300 {{ ! $readyToLoad ? 'filter blur-sm pointer-events-none select-none opacity-60' : '' }}">
