    ...
    @if($showModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 relative border border-slate-100 space-y-4">
                ...
                <h2 class="text-lg font-bold text-slate-900">{{ $editingId ? 'Rediger sagsbehandler' : 'Opret sagsbehandler' }}</h2>
                
                <form wire:submit.prevent="save" class="space-y-4">
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Navn</label>
                        <input type="text" wire:model="navn" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-800 shadow-xs focus:outline-hidden">
                        @error('navn') <p class="text-xs text-rose-600 font-medium mt-1">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" wire:model="tlf" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-800 shadow-xs focus:outline-hidden">
                        </div>
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Mobil</label>
                            <input type="text" wire:model="mobil" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-800 shadow-xs focus:outline-hidden">
                        </div>
                    </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 relative border border-slate-100 space-y-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-slate-900">Fjern sagsbehandler</h3>
                        <p class="text-xs text-slate-500">Er du sikker på, at du vil fjerne denne sagsbehandler fra kreditoren?</p>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-2 pt-3 border-t border-slate-100">
                    ...
                    <button type="button" wire:click="deleteSagsbehandler" class="px-4 py-2 text-xs font-semibold bg-rose-600 hover:bg-rose-700 text-white rounded-xl shadow-xs transition cursor-pointer">Fjern sagsbehandler</button>
                </div>
            </div>
        </div>
    @endif
