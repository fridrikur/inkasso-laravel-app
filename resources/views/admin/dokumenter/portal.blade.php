<div class="max-w-7xl mx-auto space-y-6" x-data="{ openFolders: {} }">
    ...
    {{-- GLOBAL SØGEBAR & FILTRE --}}
    <form method="GET" action="{{ route('admin.dokumenter.portal') }}" class="bg-white rounded-3xl border border-slate-200/80 p-4 shadow-xs flex flex-col sm:flex-row gap-3 items-center justify-between">
        <div class="flex items-center gap-3 w-full sm:w-1/2 pl-2">
            <span class="text-slate-400">🔍</span>
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Søg på tværs af alle filer og sagsnumre..." class="w-full text-xs text-slate-800 bg-transparent outline-none border-0 focus:ring-0">
        </div>

        <div class="flex items-center gap-2 w-full sm:w-auto justify-end">
            <select name="type" class="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-700 outline-none">
                <option value="">Alle filtyper</option>
                <option value="pdf" @selected(request('type') == 'pdf')>PDF dokumenter</option>
                <option value="xlsx" @selected(request('type') == 'xlsx')>Excel ark</option>
                <option value="jpg" @selected(request('type') == 'jpg')>Billeder (JPG/PNG)</option>
            </select>

            <button type="submit" class="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition shadow-xs cursor-pointer">
                Filtrér 🔎
            </button>

            @if(request('q') || request('type'))
                <a href="{{ route('admin.dokumenter.portal') }}" class="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-bold transition">
                    Nulstil
                </a>
            @endif
        </div>
