...
class ImportDialogsLocalCommand extends Command
{
    protected $signature = 'import:dialoger-local {--file=storage/dialoger.sql}';
    protected $description = 'Lokalt sikret kopi af den velfungerende dialog-import';

    public function handle()
    {
        $this->info('Starter lokal dialog-import (med fuld remote struktur)...');
        ...
        if (!$hasRawData) {
            $this->info('Indlæser dialoger.sql-fil via Laravel (lokal-sikret)...');
            ...
            // Læs SQL-filen direkte ind via Laravel (fungerer på alle lokale miljøer)
            ...
            $this->info('Rå dialog-tabel findes allerede lokalt. Springer fil-indlæsning over...');
        }
        ...
        // 4. Opret hoved-dialoger ved at mappe dialog.token til den legacy sager.pnummer, 
        // og find det tilsvarende sag_id ved at matche sagsnr/id mod den nye sagers-tabel.
        // 4. Opret hoved-dialoger med TRIM og BINARY for at tvinge et rent match uden mellemrumsproblemer
        $this->info('Opretter hoved-dialoger via legacy sager.pnummer = dialog.token (med TRIM)...');
        ...
        DB::statement("
            ...
            SELECT 
                ...
                NOW()
            FROM dialog d
            INNER JOIN sager s ON TRIM(s.pnummer) COLLATE utf8mb4_unicode_ci = TRIM(d.token) COLLATE utf8mb4_unicode_ci
        ...
        $this->info('Dialoger blev importeret og mappet succesfuldt lokalt med remote-logik!');
        File::put($statusFile, json_encode(['status' => 'completed', 'progress' => 100, 'message' => 'Dialoger blev importeret succesfuldt lokalt!']));
        return 0;
    }
}